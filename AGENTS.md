...
`String` canonicalizes to `Array[Byte]` in `WasmTypes::heap_type`, so both share one `(array (mut i8))` type-section entry — the distinction lives entirely in the type checker (strings are immutable and not indexable; `Array[Byte]` is neither). String literals are interned into **passive data segments** (`ir::Module::data`, deduplicated by content in `Compiler::string_data_idx`) and lower to `i32.const 0` (offset) + `i32.const len` + `array.new_data`; there is no length limit. `emit` writes the `DataCount` section before the code section (required to validate `array.new_data`) and the passive `Data` section after it, both only when segments exist — no memory section is needed. **Imports are special**: a host function can only be typed over abstract GC types, and Wasm function-type subtyping is nominal, so `WasmTypes::import_type_idx` lowers `String` params/returns in *import* signatures to the abstract `arrayref` (`ir::ValType::ArrayRef`) instead of the concrete array type; call sites stay valid because concrete arrays are subtypes of `arrayref`. Non-String imports share the ordinary deduped type entries. The runtime is configured with `wasm_gc(true)` and `wasm_function_references(true)` (see `emit::run` and the `emit`/test harness).
